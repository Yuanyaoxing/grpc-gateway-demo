...
import static io.grpc.internal.ClientCallImpl.prepareHeaders;
import static io.grpc.internal.GrpcUtil.TIMEOUT_KEY;
import static java.lang.Math.max;

/** A communication channel for making outgoing RPCs. */
@ThreadSafe
public final class GateWayChannelImpl
        extends ManagedChannel implements InternalInstrumented<InternalChannelStats> {
    static final Logger logger = Logger.getLogger(GateWayChannelImpl.class.getName());
    ...
    private final MethodDescriptor.Marshaller NONE_MARSHALLER=new MethodDescriptor.Marshaller() {
        @Override
        ...
        if (builder.idleTimeoutMillis == IDLE_TIMEOUT_MILLIS_DISABLE) {
            ...
            checkArgument(
                    builder.idleTimeoutMillis
                            >= GateWayChannelBuilder.IDLE_MODE_MIN_TIMEOUT_MILLIS,
     ...
     */
    @Override
    public GateWayChannelImpl shutdown() {
     ...
     */
    @Override
    public GateWayChannelImpl shutdownNow() {
    ...
    public void newStreamCall(InputStream req, String method, Metadata headers, CallOptions callOptions, Context context, ClientStreamListener streamListener)
    {
        ClientStream stream= NoopClientStream.INSTANCE;
        final String compressorName = callOptions.getCompressor();
        Compressor compressor = null;
        if (compressorName != null) {
            compressor = compressorRegistry.lookupCompressor(compressorName);
            if (compressor == null) {
                GrpcUtil.closeQuietly(req);
                return;
            }
        } else {
            compressor = Codec.Identity.NONE;
        }
        //去了重试机制和重试报错。估计要重新写回来
        prepareHeaders(headers, decompressorRegistry, compressor, fullStreamDecompression);
        //MethodType不知道填啥。估计要零时造一个。会检查为空。很烦。。。但是实际上用不到这些参数
        MethodDescriptor descriptor= MethodDescriptor.newBuilder().
                setFullMethodName(method).
                setSafe(false).
                setRequestMarshaller(NONE_MARSHALLER).
                setResponseMarshaller(NONE_MARSHALLER).
                setSchemaDescriptor(null).
                setType(MethodDescriptor.MethodType.UNARY).build();
        ClientTransport transport = transportProvider.get(new PickSubchannelArgsImpl(descriptor, headers, callOptions));
        //Context origContext = context.attach();
        stream = transport.newStream(descriptor, headers, callOptions);
    ...
    public String authority() {
        return nameResolver.getServiceAuthority();
    }
    ...
    private class LbHelperImpl extends LoadBalancer.Helper {
            ...
            final InternalSubchannel internalSubchannel = new InternalSubchannel(
                    ...
                    new InternalSubchannel.Callback() {
                        ...
                        void onStateChange(InternalSubchannel is, ConnectivityStateInfo newState) {
                            ...
                            if (LbHelperImpl.this == GateWayChannelImpl.this.lbHelper) {
        ...
        public String getAuthority() {
            return GateWayChannelImpl.this.authority();
        }
    ...
    private class NameResolverListenerImpl implements NameResolver.Listener {
        ...
        public void onAddresses(final List<EquivalentAddressGroup> servers, final Attributes config) {
            ...
            final class NamesResolved implements Runnable {
                ...
                public void run() {
                    // Call LB only if it's not shutdown.  If LB is shutdown, lbHelper won't match.
                    if (NameResolverListenerImpl.this.helper != GateWayChannelImpl.this.lbHelper) {
                        return;
                    }
                    try {
    ...
    @VisibleForTesting
    static final class ManagedChannelReference extends WeakReference<GateWayChannelImpl> {
        private static final ReferenceQueue<GateWayChannelImpl> refQueue =
                new ReferenceQueue<GateWayChannelImpl>();
        ...
        ManagedChannelReference(GateWayChannelImpl chan) {
            super(chan, refQueue);
